...
const Time = () => {
  ...
  function updateTime() {
    ...
    if ((time.getHours()*60) + time.getMinutes() >= 960) {
      setTimeLeft(((59 - time.getMinutes()) * 60) + ((23 - time.getHours())*3600) + (60 - time.getSeconds()) + 34200 - 1)
      setOpen(false);
    }
    // 12am-9:30am
    if ((time.getHours()*60) + time.getMinutes() < 570) {
      setTimeLeft(openTime - (time.getHours() * 3600 + time.getMinutes() * 60 + time.getSeconds())-1);
      setOpen(false);
    }
    // 9:30am - 4pm
    if ((time.getHours()* 60) + time.getMinutes() >= 570 && (time.getHours()* 60) + time.getMinutes() < 960) {
      setTimeLeft(closeTime - (time.getHours() * 3600 + time.getMinutes() * 60 + time.getSeconds())-1);
      setOpen(true);
    }
  }
